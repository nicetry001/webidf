...
function onSignIn(googleToken) {
    ...
    // Example logic for when the user successfully signs in
  ...
  function credentialExchange(googleToken) {
    ...
    if (googleTokenDecoded['sub']) {
      ...
      AWS.config.credentials = new AWS.CognitoIdentityCredentials({
        IdentityPoolId: 'us-east-1:ef70d285-f742-4f2f-b18c-286a8d0be34d', // MAKE SURE YOU REPLACE THIS
        Logins: {
  ...
  function accessImages() {
    ...
    var s3 = new AWS.S3();
    var params = {
      Bucket: "private.webidf.ar-w.info" // MAKE SURE YOU REPLACE THIS
    }; 
  
    // If we are here, things are going well, lets list all of the objects in the bucket
    s3.listObjects(params, function(err, data) {
      console.log("Listing objects in t bucket...");
      if (err) {
        document.getElementById('output').innerHTML = "<b>YOU ARE NOT AUTHORISED TO QUERY AWS!</b>";
